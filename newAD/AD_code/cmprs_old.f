...
c     
      ...
      character ans*1,dirname*80
      common /runs/ row,rnnum(200),set,ihits,icnt,nadc,obuff
      common /fil/ positn,dirname
      SET = 0
      DATA IDEV /0/
C     
...
c
      ...
      SUBROUTINE SETUP
      PARAMETER (MAXSIZE=2044)
      CHARACTER FILNAME*24,dirname*80
      INTEGER*2 ICNT, obuff(MAXSIZE), buffer, hits, ihits
      integer row, irow, rnnum, set, nadc, positn
      common /runs/ row,rnnum(200),set,ihits,icnt,nadc,obuff
      common /fil/ positn,dirname
      do 1 irow = 1,200
         rnnum(irow)=0
...
c             written to the output buffer
      ...
      INTEGER*2 ICNT, obuff(MAXSIZE), buffer, hits, ihits
      integer iflg, row, rnnum, set, nadc, positn
      common /runs/ row,rnnum(200),set,ihits,icnt,nadc,obuff
      common /fil/ positn,dirname
...
c
      ...
      PARAMETER (MAXSIZE=2044)
      CHARACTER DRVNAME*24,filname*24,file*40,file1*80,file2*80,run*5,
     +     dirname*80
      INTEGER KSKIP, IRMAX, rnnum, row, ADC(50), hitcnt, sngl,
     +     dbl, trpl, quad, mult, set, nadc, iflag, iflg, positn
      INTEGER*2 IBUF(2048), tmpstore(50), adcnum(50), numhits, tmphead,

...
C
 ...
 51	CONTINUE
	GO TO 26
C
C  START SORTING INTO 2-D arrays square
C
...
C
	...
	IF (IYM .EQ. IYP) FYP = 0.
c
	sqr(ixm,iym) = sqr(ixm,iym) + fxm*fym*frac2
	sqr(ixp,iym) = sqr(ixp,iym) + fxp*fym*frac2
	sqr(ixm,iyp) = sqr(ixm,iyp) + fxm*fyp*frac2
	sqr(ixp,iyp) = sqr(ixp,iyp) + fxp*fyp*frac2
c
150	continue
C
	go to 26
...
C
	...
	COMMON /GATES/ ITG(4),CAL(3,50),ISIZE,numgam,IADC,it(110),
     + nclvr,kclvr,calnew
      COMMON /ZX/ MAXCHN,NUMRUN,IACC,NTOTAL,IFAST,ISCRN,KZERO
	data filet/'/usr/users/tabor/sqrsort.sqr'/
...
C
	...
	jadc = 2
	ISPC = 1
1	write (ndev,*) ' Do you want the gate wide open [Y,N] ? '
	insize=1
	...
	IG4=0
	GO TO 205
100	write (ndev,*) ' TYPE MIN & MAX CHAN #S FOR ADD GATE'
	read (kdev,*) IG1,IG2
	if (ig1 .lt. 1 .or. ig2 .lt. 1) return
	IF(IG1.GT.ISIZE.OR.IG2.GT.ISIZE)GOTO100
105	write (ndev,*) ' TYPE MIN & MAX CHAN #S FOR SUB GATE',
     +  ' [or - cnts/chan ]'
	read (kdev,*) IG3,IG4
	IF(IG3.GT.ISize.OR.IG4.GT.ISIZE)GOTO105
102	write (ndev,*)' Type spectrum number and constant to add ?'
     +  ,'  [0 TO ABORT]'
	read (kdev,*) ISPC,ADD
 130    write (ndev,*) ' Type 1 to gate on x or 2 for y'
        read (kdev,*) jadc
        if (jadc .lt. 1 .or. jadc .gt. 2) return
205	IF (ISPC .EQ. 0) RETURN
	IF (ISPC .LT. 1) GO TO 102
        if (q.eq.'Y'.or.q.eq.'y') write(ndev,*)'Projecting parameter',
     +  ispc
...
200	IGD=IG4-IG3
	...
	if(ig3.lt.0.and.truet(jadc).gt.0)fac=-float(ig2-ig1+1)*ig3/truet(jadc)
	if(ig3.lt.0.and.truet(jadc).le.0) fac = 0
        if (jadc .eq. 1) go to 280
c
c     project from sqr 2-axis
c
 260    kount = 0
        count1 = 0.
        do 262 i = 1,isize
 ...
 264          tmp = tmp + sqr(i,j)
              if (ig3 .lt. 0) then
                 ...
                 do 266 j = ig3,ig4
 266                tmp = tmp - sqr(i,j)*fac
              end if
 265          tmp = tmp + add
              if (tmp .lt. 0.) tmp = 0.
              if (tmp .gt. 1000000000.) tmp = 1000000000.
              ich = ipoint(ispc) + i - 1
              idata(ich) = nint(tmp)
              kount = kount + idata(ich)
              count1 = count1 + tmp
 262          continue
              truet(ispc) = kount
              go to 320
...
c
 ...
 320          write (ndev,*) kount,' counts in spectrum'
              write (ndev,*) count1,' pairs in array'
              if (q .eq. 'N' .or. q .eq. 'n') return
              if (jadc .eq. 1) return
              ispc = 2
              jadc = 1
              go to 205

#!/usr/bin/env python3
"""
Visualize the multi-state DISCRETE fit results in a 9-panel grid.
Uses independent normalization factors for each state.
"""
import numpy as np
...
SUMMARY_FILE = "Results/independent_fits_summary.txt"
OUTPUT_PLOT = "Results/multistate_independent_summary.png"

def load_independent_norms():
    """Load independent normalization factors from summary file"""
    norms = {}
    if not os.path.exists(SUMMARY_FILE):
        print(f"Warning: {SUMMARY_FILE} not found. Using N=1.0 for all.")
        return norms
    
    with open(SUMMARY_FILE, 'r') as f:
        for line in f:
            if line.strip().startswith('#') or line.strip().startswith('State'):
                continue
            if not line.strip():
                continue
            parts = line.split()
...
def plot_all():
    ...
    for i, ax in enumerate(axes):
        ...
        # Load independent norm
        norm, norm_unc = norms.get(sid, (1.0, 0.0))
        
        # Load data
        exp, theory = load_data(sid)
        
        title_str = f"S{sid}: {desc}\nN={norm:.2f}"
        ...
        if theory is not None:
            ...
            # Plot bands (show only 2 sigma for clarity in small panels)
            ax.fill_between(angle, m2, p2, color=c_fill2, alpha=0.3, label='±2σ')
    ...
    fig.suptitle(f"36S(d,p)37S Independent Fits (Individual Normalization)", fontsize=16, fontweight='bold')

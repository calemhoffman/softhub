...
# Define chi-squared function as a function of normalization factor
def chi_squared_func(norm_factor):
    normalized_theory = norm_factor * theory_at_exp
    chi_sq = np.sum(((exp_cross_section - normalized_theory) / exp_errors)**2)
    return chi_sq
...
reduced_chi_squared = min_chi_squared / (len(exp_angles) - 1)  # N - 1 free parameters (norm factor)
...
print("=" * 70)
print("FIT WITH NORMALIZATION FACTOR")
print("=" * 70)
...
for i in range(len(exp_angles)):
    print(f"{exp_angles[i]:6.1f} {exp_cross_section[i]:10.2f} {exp_errors[i]:10.2f} "
          f"{theory_normalized[i]:10.2f} {residuals[i]:10.2f} {normalized_residuals[i]:6.2f}")
print("-" * 70)
...
ax1.set_title(f'Experimental Data vs Normalized Theory\n' + 
...
# Bottom panel: Chi-squared vs normalization factor
norm_range = np.linspace(max(0.1, optimal_norm * 0.5), optimal_norm * 1.5, 200)
chi_sq_values = [chi_squared_func(n) for n in norm_range]

ax3.plot(norm_range, chi_sq_values, 'b-', linewidth=2)
ax3.axvline(optimal_norm, color='red', linestyle='--', linewidth=2, 
            label=f'Best fit: {optimal_norm:.3f}')
ax3.axhline(min_chi_squared + 1, color='gray', linestyle=':', linewidth=1, 
            label='χ² + 1 (1σ uncertainty)')

ax3.set_xlabel('Normalization Factor', fontsize=12)
ax3.set_ylabel('χ²', fontsize=12)
ax3.set_title('Chi-squared vs Normalization Factor', fontsize=12)
ax3.legend(fontsize=10)
ax3.grid(True, alpha=0.3)

plt.savefig('fit_with_normalization.png', dpi=300, bbox_inches='tight')
print(f"\nPlot saved as 'fit_with_normalization.png'")

# Save fit results to file
with open('fit_with_normalization.txt', 'w') as f:
    f.write("FIT WITH NORMALIZATION FACTOR\n")
    f.write("=" * 70 + "\n")
...
print(f"Fit results saved to 'fit_with_normalization.txt'")
